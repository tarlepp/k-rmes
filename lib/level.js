...
Level.prototype.setApple = function setApple(data) {
  ...
  _this.algorithm = _this.options.algorithms[Math.floor(Math.random() * _this.options.algorithms.length)];
  _this.heuristic = _this.options.heuristics[Math.floor(Math.random() * _this.options.heuristics.length)];
...
Level.prototype.setReservedPoints = function setReservedPoints(grid, opponentsMovements, playerIndex) {
  ...
  if (playerIndex === _this.playerIndex) {
    ...
    Logger.info('Adding following opponent movements to be reserved points: ' + JSON.stringify(opponentsMovements));
  }
 ...
 *  winning: boolean
 *  }}                player
 * @param {boolean}   needToCalculate
 * @param {function}  next
 */
Level.prototype.calculatePaths = function calculatePaths(player, needToCalculate, next) {
  ...
  _this.setReservedPoints(grid, _this.getOpponentsPredictedMovement(), player);
  ...
  Logger.info('New path calculated to apple, length: ' + player.path.length + ' time: ' + player.pathTime + 'ms');
 ...
 * @returns {Array}
 */
Level.prototype.getOpponentsPredictedMovement = function getOpponentsPredictedMovement() {
